...
ImageInfo(){;}
ImageInfo(int width,int height,unsigned int texture){
	this->width = width;
this->height = height;
...
;
ImageInfo iLoadImage2(char filename[])
{
	int width, height, bpp;
	...
	return ImageInfo(width,height,texture);
}

void iShowImage2(int x, int y, ImageInfo imageinfo)
{
	int width = imageinfo.width, height = imageinfo.height;
	unsigned int texture = imageinfo.texture;
	//unsigned int texture;
